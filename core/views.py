...
# Función para obtener el cliente de MongoDB (sin cambios)
...
# Decorador para proteger vistas (sin cambios)
...
# Vista para insertar cliente (sin cambios)
@mongo_login_required
...
# Vista para insertar pedido (actualizada con cantidades)
@mongo_login_required
...
# Filtros según requerimientos (sin cambios)
@mongo_login_required
...
def filter_pedidos_producto_101(request):
    client = get_mongo_client(request)
    db = client['ecommerce_db']
    query = {'productos.producto_id': '101'}  # Compatible con IDs como strings
    pedidos_list = list(db['pedidos'].find(query))
    return render(request, 'core/filter_pedidos.html', {'pedidos': pedidos_list})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if db['productos'].find_one({'_id': id_producto}):
                ...
                producto_data['_id'] = id_producto  # Map id_producto to _id for MongoDB
                del producto_data['id_producto']    # Remove id_producto from the data
